...
function DetailJurnalContent() {
  ...
  const { activities, setEditingActivity, setActivityCategoryPreset, triggerNotification, setIsLoading, setLoadingMsg } = useApp();
  ...
  const valueEntries = coreValuesList.map(v => {
    const matchingActivity = currentMonthActivities.find(act => getCoreValueFromTitle(act.title) === v.name);
    return {
      value: v.name,
      icon: v.icon,
  ...
  const filledCount = valueEntries.filter(item => item.activity !== null).length;
      ...
      <section className="flex flex-col gap-4">
        ...
        <div className="flex flex-col gap-3">
          {valueEntries.map((item) => {
            const hasEntry = item.activity !== null;
            ...
            return (
              ...
              >
                  ...
                  <div className="flex items-center gap-3 overflow-hidden">
                    ...
                    <div className="overflow-hidden">
                      ...
                      <p className="text-[10px] text-outline mt-0.5">
                        {hasEntry ? "Sudah Diisi" : "Belum Diisi"}
                      </p>
                    </div>
                  </div>
                ...
                {/* Collapsible Details */}
                {hasEntry && isExpanded && act && (
                  <div className="px-3.5 pb-4 pt-1 border-t border-outline-variant/20 bg-surface-container-low/20 flex flex-col gap-3">
                    ...
                    <div className="flex justify-end gap-2 pt-2 border-t border-outline-variant/20 mt-1">
                      ...
                      <button
                        ...
                        className="px-4 py-2 rounded-lg bg-primary text-on-primary font-label-md text-xs font-bold hover:bg-primary-container active:scale-[0.98] transition-all cursor-pointer border-none"
                      >
                        Edit Entri
                      </button>
                    </div>
